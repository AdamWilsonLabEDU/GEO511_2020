...
* Adam out of town (Biodiversity Workshop)
* Tuesday 
...
## Reproject to ["Equal Earth" projection](http://equal-earth.com/equal-earth-projection.html)
```{r, fig.height=3}
data(world) # from spData package
st_crs(world)
world_albers=world %>% 
  st_transform(crs = "+proj=eqearth +wktext")
p1 <- ggplot(world,aes(fill=gdpPercap))+geom_sf()
p2 <- ggplot(world_albers,aes(fill=gdpPercap))+geom_sf()
