...
buffalo <- get_decennial(geography = "block", variables = racevars, 
...
buffalo_dot <- buffalo %>% 
   st_crop(c(xmin=-78.9,xmax=-78.7,ymin=42.85,ymax=42.95)) %>% 
   st_transform(proj) %>% 
   st_sample(size=buffalo$value)

n=1000
world_points=st_sample(world2,size=n)
plot(world_points)
